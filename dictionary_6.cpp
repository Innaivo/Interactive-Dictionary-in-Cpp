    ...
    {/* find a similar word from dictionary: */
     ...
     if (matchWordpair.size() != 0)
      { char answer = 0;
        ...
        cin >> answer;
        if (answer == 'y') defVect = check_conditions(matchWordpair[0]);
        else
        { if (answer == 'n')
          {defVect = {"The word doesn't exist, yet."};}
          else
          {defVect = {"We don't understand your entry. Apologies."};}
        }
      } /*if there is at least 1 match with given threshold*/
    }/* if func. check_conditions gave zero-output 1st time*/
